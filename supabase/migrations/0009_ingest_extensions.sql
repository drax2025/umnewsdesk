...
alter table public.candidates
  add column external_id  text,            -- source's own ID (RSS guid, email message-id, etc.)
  add column kind         text,            -- 'rss' | 'email' | 'pdf' | 'web' | 'generic'
  add column summary      text,
  add column body_text    text,
  add column author       text,
  add column tags         text[]  not null default '{}',
  add column raw          jsonb,           -- the original normalized envelope, for audit / re-processing
  add column fetched_at   timestamptz,     -- when the ingestion runner pulled it
  add column published_at timestamptz;     -- when the source claims it was published

-- Dedup helpers
create unique index candidates_source_external_idx
  on public.candidates(source_id, external_id)
  where external_id is not null;

create index candidates_primary_url_idx
  on public.candidates(primary_url)
  where primary_url is not null;

-- Fuzzy headline dedup uses pg_trgm
create extension if not exists pg_trgm;
create index candidates_headline_trgm_idx
  on public.candidates using gin (working_headline gin_trgm_ops);

-- ─── sweep_runs ─────────────────────────────────────────────
alter table public.sweep_runs
  add column trigger text not null default 'scheduled';   -- 'scheduled' | 'manual' | 'webhook'
...
create index ops_rr_alerts_open_dedup_idx
